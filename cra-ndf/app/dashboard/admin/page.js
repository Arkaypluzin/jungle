...
export default function AdminCRAPage() {
  ...
  const fetchActivityTypes = useCallback(async () => {
    try {
      ...
      console.log("AdminCRAPage: Activity Type Definitions fetched:", activityTypesData); // Log activity types
  ...
  const handleShowDetailedReport = useCallback(() => {
    ...
    const totalPaidLeaveDaysInMonth = craActivities.reduce((sum, activity) => {
      // Find the activity type name for the current activity
      const activityTypeName = activityTypeDefinitions.find(
        (type) => String(type.id) === String(activity.activity_type_id)
      )?.name;

      if (activityTypeName === "Congé Payé") {
        return sum + (parseFloat(activity.duree_jours) || 0);
      }
      return sum;
    }, 0);
    ...
    craActivities,
    clientDefinitions,
    activityTypeDefinitions, // Added to dependency array
    currentDisplayedMonth,
    currentUserName,
  ]);
