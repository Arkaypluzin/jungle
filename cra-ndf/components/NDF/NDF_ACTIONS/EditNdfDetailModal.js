...
export default function EditNdfDetailModal({ detail, onEdited, parentNdfMonth, parentNdfYear }) {
  ...
  useEffect(() => {
    if (tva === "multi-taux") setMontant(totalHTMulti.toFixed(2));
  }, [totalHTMulti, tva]);
  ...
  async function handleSubmit(e) {
    ...
    // Blocage si date hors du mois/année
    if (dateStr) {
      ...
      if (
        ...
        setError("La date choisie doit appartenir au mois et à l'année de la note de frais.");
        setLoading(false);
        return;
      }
    }
    ...
    let extra = {};
    if (tva === "autre taux") tvaValue = autreTaux;
                ...
                <div className="pt-4 border-t border-gray-200 mt-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {projets.map(p => (
                        <option key={p.id || p.uuid} value={p.id || p.uuid}>{p.nom}</option>
                      ))}
                    </select>
                  </div>
                </div>
