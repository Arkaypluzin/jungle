...
export default function SummaryReport({
  ...
  timeDifference,
  onClose, // La fonction pour fermer le rapport de synthèse
  onOpenMonthlyReportPreview, // Nouvelle prop: fonction pour ouvrir le rapport détaillé
}) {
  // Supprimez les états et useEffect liés à l'exportation PDF directe d'ici.
  // const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);
  // const [scriptsLoaded, setScriptsLoaded] = useState(false);
  // useEffect pour le chargement des scripts PDF a été déplacé ou supprimé d'ici.

  // Vérifier si currentMonth est une date valide avant de l'utiliser
  const displayMonth = isValid(currentMonth)
    ? format(currentMonth, "MMMM", { locale: fr }) // Format complet avec l'année
    : "Mois inconnu";
  ...
  // Fonction pour gérer le clic sur le bouton de fermeture du rapport de synthèse
  const handleCloseClick = useCallback(() => {
    console.log(
      "SummaryReport: Le bouton 'Fermer' a été cliqué. Appel de la prop onClose."
    );
    onClose(); // Appelle la fonction onClose passée par le composant parent (CraBoard)
  }, [onClose]);

  // Nouvelle fonction pour gérer le clic sur le bouton "Voir le rapport détaillé"
  const handleViewDetailedReport = useCallback(() => {
    console.log(
      "SummaryReport: Le bouton 'Voir le rapport détaillé' a été cliqué."
    );
    // Appelle la fonction passée par CraBoard pour ouvrir la modal de prévisualisation
    if (onOpenMonthlyReportPreview) {
      onOpenMonthlyReportPreview();
    }
    onClose(); // Ferme le rapport de synthèse après avoir ouvert la prévisualisation
  }, [onOpenMonthlyReportPreview, onClose]);

  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center p-4 z-50">
      <div
        id="summary-report-container"
        className="bg-white rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-4xl max-h-[90vh] overflow-y-auto relative"
      >
        <button
        ...
        <div id="summary-report-content">
          {" "}
          {/* Conservez cet ID si vous voulez le cibler pour un futur export direct du résumé */}
        ...
        {/* Fin de summary-report-content */}
        <div className="text-center mt-6 flex justify-center space-x-4">
          {/* Nouveau bouton pour ouvrir le rapport détaillé */}
          <button
