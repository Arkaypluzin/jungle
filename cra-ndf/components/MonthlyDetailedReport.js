...
export default function MonthlyDetailedReport({ reportData, userId, year, month, userName }) {
  ...
  const totalDaysForMonth = useMemo(() => {
    return reportData.reduce((sum, activity) => sum + (parseFloat(activity.temps_passe) || 0), 0).toFixed(1);
  }, [reportData]);

  return (
    <div className="bg-white p-8 rounded-lg shadow-xl max-w-5xl mx-auto my-8 print:shadow-none print:my-0 print:p-0">
      <div className="text-center mb-8 print:mb-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2 print:text-2xl">Rapport d'activité Mensuel Détaillé</h1>
        <h2 className="text-xl text-gray-700 mb-1 print:text-lg">
          Pour {userName} - {format(new Date(year, month - 1), 'MMMM', { locale: fr })}
        </h2>
        <p className="text-md text-gray-600 print:text-sm">Total général pour le mois : {totalDaysForMonth} jours</p>
      </div>

      {Object.keys(groupedActivities).length === 0 ? (
        <p className="text-center text-gray-600 py-8">Aucune activité trouvée pour ce mois-ci.</p>
      ) : (
        <div className="space-y-6 print:space-y-3">
          {Object.keys(groupedActivities).map(dateKey => {
            ...
            return (
              <div key={dateKey} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm print:border-none print:shadow-none">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50 print:bg-white">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-sm">Type</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-sm">Client</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-sm">Description</th>
                        <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider print:text-sm">Temps (jours)</th>
                        <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider print:text-sm">Facturable</th>
                      </tr>
                    </thead>
