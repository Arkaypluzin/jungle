// components/ClientMonthlyReport.js
...
export default function ClientMonthlyReport({
  ...
  const groupedActivities = useMemo(() => {
    ...
    // Sort activities within each day by type
    const sortedGroups = {};
  ...
  const totalDaysForClient = useMemo(() => {
    return reportData
      .reduce(
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-xl max-w-4xl mx-auto my-8 print:shadow-none print:my-0 print:p-0">
      <div className="text-center mb-8 print:mb-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2 print:text-2xl">
          Fiche d'activité Mensuelle
        </h1>
        ...
        <p className="text-md text-gray-600 print:text-sm">
          Total jours pour ce client : {totalDaysForClient}
        </p>
      </div>

      {Object.keys(groupedActivities).length === 0 ? (
        <p className="text-center text-gray-600 py-8">
          Aucune activité trouvée pour ce client ce mois-ci.
        </p>
      ) : (
      ...
      <div className="mt-8 text-right text-xl font-bold text-gray-800 border-t pt-4 print:mt-4 print:pt-2 print:text-lg">
        Total général pour {clientName} : {totalDaysForClient} jours
      </div>
